...
test("classifies every executable test entry exactly once in an explicit layer", () => {
  ...
  assert.equal(audit.entryCount, 102)
  ...
  assert.equal(audit.assignments["tests/ohos-typescript-spike.test.mjs"], "unit-contract")
  ...
  assert.equal(
    audit.assignments["tests/semantic/semantic-coordinator.test.mjs"],
    "unit-contract",
  )
  assert.equal(
    audit.assignments["tests/semantic/type-engine-context-runtime.test.mjs"],
    "unit-contract",
  )
  assert.equal(
  ...
  assert.equal(audit.assignments["tests/toolchain-lock.test.mjs"], "unit-contract")
  assert.equal(audit.assignments["tests/upstream-identity.test.mjs"], "unit-contract")
  assert.equal(audit.assignments["tests/workspace-path-consistency.test.mjs"], "unit-contract")
  assert.equal(
  ...
  assert.equal(
    audit.assignments["tests/lsp-production-semantic-worker.test.mjs"],
    "bundle-e2e",
  )
  assert.equal(
